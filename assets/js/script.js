...
(function () {
 ...
 // ================================================================
// SCROLL REVEAL (IntersectionObserver) - kama kwenye damygasotech.com
// ================================================================
// Njia hii ni imara zaidi kuliko kupima getBoundingClientRect kwa
// setTimeout: haitegemei muda, na inafanya kazi kiotomatiki hata kadi
// zikiwa ndani ya page iliyofichwa (display:none), kwa sababu observer
// hupima upya mara element inapoonekana.
//
// Inashughulikia sehemu ZOTE (Projects, Services, Timeline, Skills,
// About, Contact form, n.k) - pamoja na content inayoongezwa baadaye
// kwa JS (mfano testimonials kutoka Google Reviews), kupitia
// MutationObserver.
(function () {
  ...
  // Vivinjari vya zamani visivyo na IntersectionObserver - onyesha vyote
  if (!('IntersectionObserver' in window)) {
    document.querySelectorAll(REVEAL_SELECTOR).forEach(el => el.classList.add('reveal-visible'));
    return;
  }
  ...
  // Content inayoongezwa baadaye (mf. testimonials kutoka Google Reviews)
  // inapoongezwa kwenye DOM, tuiangalie pia.
  const domWatcher = new MutationObserver(mutations => {
    mutations.forEach(mutation => {
      mutation.addedNodes.forEach(node => {
        if (node.nodeType !== 1) return;
        if (node.matches && node.matches(REVEAL_SELECTOR)) observeReveal(node.parentElement || document);
        observeReveal(node);
      });
    });
  });
...
// ================================================================
// PORTFOLIO LOADER - loader nyepesi wakati tab ya Portfolio
// inapofunguliwa, kusubiri picha za miradi zipakie
// ================================================================
(function () {
  const loader = document.querySelector('[data-portfolio-loader]');
  const list = document.querySelector('.project-list');
  if (!loader || !list) return;

  function runPortfolioLoader() {
    const viewportH = window.innerHeight;

    // Loader inasubiri picha za kadi zinazoonekana kwenye skrini pekee
    // (above the fold) - siyo zote, kwa sababu nyingi zina loading="lazy"
    // na hazitapakia mpaka mtumiaji ascroll, jambo ambalo lingefanya
    // loader ikae muda mrefu bila sababu.
    const visibleImgs = Array.from(list.querySelectorAll('.project-item.active img')).filter(img => {
      const rect = img.closest('.project-item').getBoundingClientRect();
      return rect.top < viewportH && rect.bottom > 0;
    });
    const pendingImgs = visibleImgs.filter(img => !img.complete);
    ...
    // Chelewesha kuonyesha loader kidogo - ikiwa picha zitapakia haraka
    // (mf. tayari zipo kwenye cache), loader haitaonekana kabisa (hakuna
    // "flash" isiyo na maana).
    const showTimer = setTimeout(() => loader.classList.remove('hidden'), 150);

    // Usalama: loader isikwame milele kama picha moja imeshindwa kupakia
    ...
    Promise.all(
      pendingImgs.map(img => new Promise(resolve => {
  ...
  document.querySelectorAll('[data-nav-link]').forEach(link => {
    link.addEventListener('click', function () {
      if (this.dataset.target === 'portfolio') {
        runPortfolioLoader();
      }
    });
  });

  // Filter ikibadilishwa, kadi mpya zinaweza kuwa na picha ambazo
  // hazijapakia bado
  document.querySelectorAll('[data-filter-btn], [data-select-item]').forEach(btn => {
